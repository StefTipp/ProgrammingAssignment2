...
## makeCacheMatrix receives an input matrix x, which will then be saved in the cache; the output list contains the four 
## different functions
...
makeCacheMatrix <- function(x = matrix()) {
  ...
  setmatrix<-function(solve) m<<- solve # saves the inverse to m in the enclosing environment / caches it
...
## cacheSolve retrieves the inverse of the input matrix x from the cache if has been calculated already,
